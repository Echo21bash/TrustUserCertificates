...
exec > /data/local/tmp/trust_usercert/trust_usercert.log
exec 2>&1
...
ls /data/misc/user/*/cacerts-added/* | grep -o -E '[0-9a-fA-F]{8}.[0-9]+$' | cut -d '.' -f1 | sort | uniq > /data/local/tmp/trust_usercert/user_certs.txt

echo "[i] found $(cat /data/local/tmp/trust_usercert/user_certs.txt | wc -l) certificates"
...
if [ -d /apex/com.android.conscrypt/cacerts ]; then
    ...
    mkdir -p /data/local/tmp/trust_usercert/ca
    if mountpoint /data/local/tmp/trust_usercert/ca;then
      echo "[i] /data/local/tmp/trust_usercert/ca mounted"
      cp -rf /apex/com.android.conscrypt/cacerts/* /data/local/tmp/trust_usercert/ca/
    else
      rm -rf /data/local/tmp/trust_usercert/ca/*
      mount -t tmpfs tmpfs /data/local/tmp/trust_usercert/ca
      cp -rf /apex/com.android.conscrypt/cacerts/* /data/local/tmp/trust_usercert/ca/
    fi
else
...
while read USER_CERT_HASH; do
    ...
    if [ -d /apex/com.android.conscrypt/cacerts ]; then
        ...
        cp -f ${USER_CERT_FILE} /data/local/tmp/trust_usercert/ca/${USER_CERT_HASH}.0
    fi

done </data/local/tmp/trust_usercert/user_certs.txt

# Android 14+ support

echo "[i] checking for cert location in APEX container"
if [ -d /apex/com.android.conscrypt/cacerts ]; then
    chown -R 0:0 /data/local/tmp/trust_usercert/ca
    set_context /apex/com.android.conscrypt/cacerts /data/local/tmp/trust_usercert/ca
    ...
    if mountpoint /apex/com.android.conscrypt/cacerts;then
      ...
      mount --bind /data/local/tmp/trust_usercert/ca /apex/com.android.conscrypt/cacerts && \
      echo "[i] /apex/com.android.conscrypt/cacerts mount succeeded"
    fi
    for pid in 1 $(pgrep zygote) $(pgrep zygote64); do
      nsenter --mount=/proc/${pid}/ns/mnt -- \
      /bin/mount --bind /data/local/tmp/trust_usercert/ca /apex/com.android.conscrypt/cacerts
    done
fi
